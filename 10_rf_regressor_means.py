...
def pred_rfr_mean(train,test):
    ...
    test_data_completa = test_entrega.dropna()
    
    test_data_incompleta = test_entrega[~test_entrega.index.isin(test_data_completa.index)]
     
    
    test_data_incompleta['rating_ambiente_pred'] = (np.where(np.isnan(test_data_incompleta.usuario_mean_ambiente)==False,test_data_incompleta['usuario_mean_ambiente'],np.where(np.isnan(test_data_incompleta.rest_mean_ambiente)==False,test_data_incompleta['rest_mean_ambiente'],global_mean_ambiente)))
    test_data_incompleta['rating_comida_pred'] = (np.where(np.isnan(test_data_incompleta.usuario_mean_comida)==False,test_data_incompleta['usuario_mean_comida'],np.where(np.isnan(test_data_incompleta.rest_mean_comida)==False,test_data_incompleta['rest_mean_comida'],global_mean_comida)))
    test_data_incompleta['rating_servicio_pred'] = (np.where(np.isnan(test_data_incompleta.usuario_mean_servicio)==False,test_data_incompleta['usuario_mean_servicio'],np.where(np.isnan(test_data_incompleta.rest_mean_servicio)==False,test_data_incompleta['rest_mean_servicio'],global_mean_servicio)))
    
    
    rf_ambiente = joblib.load('rf_ambiente.pkl') 
    
    pred_ambiente =  rf_ambiente.predict(test_data_completa[cols])

    
    ##COMIDA
    rf_comida = joblib.load('rf_comida.pkl') 
    
    pred_comida =  rf_comida.predict(test_data_completa[cols])
    ...
    pred_servicio =  rf_servicio.predict(test_data_completa[cols])
    ...
    test_data_pred = pd.concat([pd.concat([test_data_completa,test_data_incompleta]).sort_index(),test['fecha']],axis=1)
    
    
    
    test_data_pred = test_data_pred[['id_usuario','id_restaurante','fecha','rating_ambiente_pred','rating_comida_pred','rating_servicio_pred']]
    test_data_pred.columns = ['id_usuario','id_restaurante','fecha_int','fecha','rating_ambiente','rating_comida','rating_servicio']
    test_data_pred = test_data_pred.drop('fecha_int',axis=1)

    return test_data_pred


entrega = pred_rfr_mean(train,test)

entrega.to_csv('pablot-09-rfr_mean.csv',index=False)
